<template>
    <div class="
        ...
        <table class="table-auto border border-separate
            ...
            <tr class="bg-gray-200 even:bg-gray-50 hover:bg-gray-200 cursor-pointer" v-for="fila in filas">
                <td class="
                            border-b-2 border-gray-500
                             px-4 p-2 text-xl text-gray-500  divide-green-600 " v-for="valor in fila">
                    {{ valor }}
                </td>
                <td v-if="rol == 'administrador'" @click="editar(fila.id)"
                    class="text-center self-center  pt-6 h-10v w-5v  fa fa-pencil" aria-hidden="true">
                </td>
                <td v-if="rol == 'administrador'" @click="borrar(fila.id)"
                    class="text-center self-center  pt-6  h-10v w-5v fas fa-trash-alt" aria-hidden="true">
                </td>
                <td aria-hidden="true">
                    <button @click="consultar(id)"></button>
                </td>
            </tr>
        </table>
    </div>
